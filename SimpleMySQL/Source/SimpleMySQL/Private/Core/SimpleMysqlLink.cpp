...
int32 FSimpleMysqlLink::GetLastInsertID()
{
	if (Mysql)
	{
...
bool FSimpleMysqlLink::QueryLinkStmt(const FString& SQL, const TArray<FSimpleMysqlQueryStmt> &InQueryStmtParam, FSimpleMysqlQueryStmtResult& OutResult, FString& ErrMesg)
{
	...
	{
		//绑定Stmt参数 可以不用指定参数 根据需求
		PrepareStmtParameters(InQueryStmtParam, PrepareParametersError);

		bQuerySuccess = ExecuteStmt(OutResult, ExecuteStmtError);
	}
...
bool FSimpleMysqlLink::ExecuteStmt(FSimpleMysqlQueryStmtResult& OutResult,FString& ErrMesg)
{
	if (mysql_stmt_execute(MysqlStmt) == 0)
	{
		...
		OutResult.InsertId = mysql_stmt_insert_id(MysqlStmt);
		OutResult.AffectedRows = mysql_stmt_affected_rows(MysqlStmt);
		OutResult.PreparedStatementID =  MysqlStmt->stmt_id;
...
bool FSimpleMysqlLink::QueryLink(const FString &SQL,FString &ErrMesg)
{
	...
	if (Ret == 0)
	{
		...
		{
			if ((Ret = mysql_real_query(Mysql, TCHAR_TO_UTF8(*SQL), SQL.Len())) != 0)
			{
				ErrMesg = UTF8_TO_TCHAR(mysql_error(Mysql));
				return false;
			}
	...
	while (MYSQL_ROW SQLRow = mysql_fetch_row(Result))
	{
		...
		for (int i = 0; i < NumRow; i++)
		{
			if (SQLRow[i] != NULL)
			{
				...
				FString c_value = UTF8_TO_TCHAR(o);
				FString c_name = UTF8_TO_TCHAR(mysql_fetch_field_direct(Result, i)->name);

				if (!c_name.IsEmpty())
				{
					if (c_value.IsEmpty())
					{
						R->Rows.Add(c_name, TEXT(""));
					}
					else
					{
						R->Rows.Add(c_name, c_value);
					}
				}	
			}
			else
